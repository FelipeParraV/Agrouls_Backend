<?php
    ...
    switch ($method) {
        case 'GET':
            ...
            $sql = "SELECT * FROM actividades ORDER BY orden";
        ...
        case 'POST':
            ...
            if ($action === 'new') {
                ...
                $sql = "SELECT orden FROM actividades ORDER BY orden DESC LIMIT 1"; // Consulta SQL para obtener el último orden
                $result = $conn->query($sql);
                ...
                $sql = "INSERT INTO actividades (titulo, imagen, orden) VALUES ('$titulo', '$image_path', '$orden')";
                ...
                if(isset($_FILES['image'])) {
                    ...
                    // Si todo está bien, intenta subir el archivo
                        ...
                        if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) { // Si el archivo se sube correctamente
                            ...
                            $sql = "SELECT imagen FROM actividades WHERE id=$id";
                            $result = $conn->query($sql);
                            if ($result->num_rows > 0) {
                                $row = $result->fetch_assoc();
                            ...
                            // Insertar los datos en la base de datos
                            $sql = "UPDATE actividades SET imagen='$image_path' WHERE id=$id";
                ...
                foreach ($items as $item) {
                    $id = $item['id'];
                    $order = $item['order'];
                    $sql = "UPDATE actividades SET orden = ? WHERE id = ?";
        ...
        case 'PUT':
            ...
            $sql = "UPDATE actividades SET titulo='$titulo' WHERE id=$id";
        ...
        case 'DELETE':
            ...
            $sql = "DELETE FROM actividades WHERE id=$id"; // Consulta SQL para eliminar el registro
            if ($conn->query($sql) === TRUE) { // Si la consulta se ejecuta correctamente
