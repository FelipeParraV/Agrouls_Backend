<?php
    ...
    switch ($method) {
        case 'GET':
            ...
            $sql = "SELECT * FROM escuela_3 ORDER BY orden";
        ...
        case 'POST':
            ...
            if ($action === 'new') {
                $sql = "SELECT orden FROM escuela_3 ORDER BY orden DESC LIMIT 1"; // Consulta SQL para obtener el último orden
                ...
                $cargo = $_POST['cargo']; // Obtiene el título de la noticia

                // Insertar los datos en la base de datos
                $sql = "INSERT INTO escuela_3 (nombre, cargo, orden) VALUES ('$nombre', '$cargo', '$orden')";
                ...
                foreach ($items as $item) {
                    $id = $item['id'];
                    $order = $item['order'];
                    $sql = "UPDATE escuela_3 SET orden = ? WHERE id = ?";
        ...
        case 'PUT':
            ...
            $cargo = $_PUT['editCargo']; // Obtiene el título de la noticia


            $sql = "UPDATE escuela_3 SET nombre='$nombre', cargo='$cargo' WHERE id=$id";
        ...
        case 'DELETE':
            ...
            $sql = "DELETE FROM escuela_3 WHERE id=$id"; // Consulta SQL para eliminar el registro
            if ($conn->query($sql) === TRUE) { // Si la consulta se ejecuta correctamente
