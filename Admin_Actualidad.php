<?php
    ...
    switch ($method) {
        case 'GET':
            ...
            $sql = "SELECT * FROM actualidad ORDER BY orden";
        ...
        case 'POST':
            ...
            if ($action === 'new') {
                if(isset($_FILES['image'])) { // Verificar si se ha subido un archivo
                    ...
                    // Si todo está bien, intenta subir el archivo
                        ...
                        if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) { // Si el archivo se sube correctamente
                            ...
                            $sql = "SELECT orden FROM actualidad ORDER BY orden DESC LIMIT 1"; // Consulta SQL para obtener el último orden
                            ...
                            $fecha = $_POST['fecha'];  // Obtiene la fecha de la noticia
                            $enunciado = $_POST['enunciado'];  // Obtiene la fecha de la noticia
                            $descripcion = $_POST['descripcion']; // Obtiene la descripción de la noticia
                            $image_path = $image_name; // Obtiene el nombre del archivo

                            //echo "Variables: titulo=$titulo, fecha=$fecha, descripcion=$descripcion, imagen=$image_path";
                            // Insertar los datos en la base de datos
                            $sql = "INSERT INTO actualidad (titulo, fecha, enunciado, descripcion, imagen, orden) VALUES ('$titulo', '$fecha', '$enunciado', '$descripcion', '$image_path', '$orden')";
                ...
                if(isset($_FILES['image'])) {
                    ...
                    // Si todo está bien, intenta subir el archivo
                        ...
                        if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) { // Si el archivo se sube correctamente
                            ...
                            $sql = "SELECT imagen FROM actualidad WHERE id=$id";
                            $result = $conn->query($sql);
                            if ($result->num_rows > 0) {
                                $row = $result->fetch_assoc();
                            ...
                            // Insertar los datos en la base de datos
                            $sql = "UPDATE actualidad SET imagen='$image_path' WHERE id=$id";
                ...
                foreach ($items as $item) {
                    $id = $item['id'];
                    $order = $item['order'];
                    $sql = "UPDATE actualidad SET orden = ? WHERE id = ?";
        ...
        case 'PUT':
            ...
            $enunciado = $_PUT['editEnunciado']; // Obtiene el título de la noticia
            $fecha = $_PUT['editFecha'];  // Obtiene la fecha de la noticia
            $descripcion = $_PUT['editDescripcion']; // Obtiene la descripción de la noticia

            echo "Variables: id=$id, titulo=$titulo, fecha=$fecha, descripcion=$descripcion";

            $sql = "UPDATE actualidad SET titulo='$titulo', fecha='$fecha', enunciado='$enunciado', descripcion='$descripcion' WHERE id=$id";
        ...
        case 'DELETE':
            ...
            $sql = "DELETE FROM actualidad WHERE id=$id"; // Consulta SQL para eliminar el registro
            if ($conn->query($sql) === TRUE) { // Si la consulta se ejecuta correctamente
