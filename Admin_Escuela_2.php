<?php
    ...
    switch ($method) {
        case 'GET':
            ...
            $sql = "SELECT * FROM escuela_2 ORDER BY orden";
        ...
        case 'POST':
            ...
            if ($action === 'new') {
                $sql = "SELECT orden FROM escuela_2 ORDER BY orden DESC LIMIT 1"; // Consulta SQL para obtener el último orden
                ...
                $proposito = $_POST['proposito']; // Obtiene el título de la noticia

                // Insertar los datos en la base de datos
                $sql = "INSERT INTO escuela_2 (proposito, orden) VALUES ('$proposito', '$orden')";
                ...
                foreach ($items as $item) {
                    $id = $item['id'];
                    $order = $item['order'];
                    $sql = "UPDATE escuela_2 SET orden = ? WHERE id = ?";
        ...
        case 'PUT':
            ...
            $id = $_PUT['editId'];
            $proposito = $_PUT['editProposito']; // Obtiene el título de la noticia


            $sql = "UPDATE escuela_2 SET proposito='$proposito' WHERE id=$id";
        ...
        case 'DELETE':
            ...
            $sql = "DELETE FROM escuela_2 WHERE id=$id"; // Consulta SQL para eliminar el registro
            if ($conn->query($sql) === TRUE) { // Si la consulta se ejecuta correctamente
