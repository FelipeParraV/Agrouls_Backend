<?php
    ...
    switch ($method) {
        case 'GET':
            ...
            $sql = "SELECT * FROM datos ORDER BY orden";
        ...
        case 'POST':
            ...
            if ($action === 'new') {
                ...
                $sql = "SELECT orden FROM datos ORDER BY orden DESC LIMIT 1"; // Consulta SQL para obtener el último orden
                $result = $conn->query($sql);
                ...
                $valor = $_POST['valor']; // Obtiene la descripción de la noticia
                $image_path = $image_name; // Obtiene el nombre del archivo

                // Insertar los datos en la base de datos
                $sql = "INSERT INTO datos (titulo, valor, imagen, orden) VALUES ('$titulo', '$valor', '$image_path', '$orden')";
                ...
                if(isset($_FILES['image'])) {
                    ...
                    // Si todo está bien, intenta subir el archivo
                        ...
                        if (move_uploaded_file($_FILES["image"]["tmp_name"], $target_file)) { // Si el archivo se sube correctamente
                            ...
                            $sql = "SELECT imagen FROM datos WHERE id=$id";
                            $result = $conn->query($sql);
                            if ($result->num_rows > 0) {
                                $row = $result->fetch_assoc();
                            ...
                            // Insertar los datos en la base de datos
                            $sql = "UPDATE datos SET imagen='$image_path' WHERE id=$id";
                ...
                foreach ($items as $item) {
                    $id = $item['id'];
                    $order = $item['order'];
                    $sql = "UPDATE datos SET orden = ? WHERE id = ?";
        ...
        case 'PUT':
            ...
            $valor = $_PUT['editValor']; // Obtiene la descripción de la noticia


            $sql = "UPDATE datos SET titulo='$titulo', valor='$valor' WHERE id=$id";
        ...
        case 'DELETE':
            ...
            $sql = "DELETE FROM datos WHERE id=$id"; // Consulta SQL para eliminar el registro
            if ($conn->query($sql) === TRUE) { // Si la consulta se ejecuta correctamente
