<?php
    ...
    switch ($method) {
        case 'GET':
            ...
            $sql = "SELECT * FROM perfil_1 ORDER BY orden";
        ...
        case 'POST':
            ...
            if ($action === 'new') {
                $sql = "SELECT orden FROM perfil_1 ORDER BY orden DESC LIMIT 1"; // Consulta SQL para obtener el último orden
                ...
                $capacidades = $_POST['capacidades']; // Obtiene el título de la noticia

                // Insertar los datos en la base de datos
                $sql = "INSERT INTO perfil_1 (capacidades, orden) VALUES ('$capacidades', '$orden')";
                ...
                foreach ($items as $item) {
                    $id = $item['id'];
                    $order = $item['order'];
                    $sql = "UPDATE perfil_1 SET orden = ? WHERE id = ?";
        ...
        case 'PUT':
            ...
            $id = $_PUT['editId'];
            $capacidades = $_PUT['editCapacidades']; // Obtiene el título de la noticia


            $sql = "UPDATE perfil_1 SET capacidades='$capacidades' WHERE id=$id";
        ...
        case 'DELETE':
            ...
            $sql = "DELETE FROM perfil_1 WHERE id=$id"; // Consulta SQL para eliminar el registro
            if ($conn->query($sql) === TRUE) { // Si la consulta se ejecuta correctamente
